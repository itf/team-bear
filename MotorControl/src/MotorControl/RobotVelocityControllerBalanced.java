...
public class RobotVelocityControllerBalanced extends RobotVelocityController {
  ...
  public void controlStep(double[] controlOutput) {
    ...
    //code for Motor Control, Lab 2. Spring 2015

    // Begin Student Code
    
    angularDiffError= this.desiredDiffAngle - this.actualDiffAngle; //positive: left turned more than it should have
    ldv=this.getDesiredAngularVelocity(RobotBase.LEFT);
    rdv=this.getDesiredAngularVelocity(RobotBase.RIGHT);
//    if ((updateDbg++ % 20) == 0) {
//        System.err.println("pre gain desireds: ldv: " + ldv + "; rdv: " + rdv);
//      }
   // if (angularDiffError > 0) 
    	ldv += gain * angularDiffError;
    
   // if (angularDiffError < 0)
    	rdv -= gain * angularDiffError;
