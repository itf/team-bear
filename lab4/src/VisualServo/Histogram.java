	...
	public static float[][] makeHistogram(Image source, boolean hsbHistogram) {
		...
		if (hsbHistogram) {
			...
			for (int x=0; x<width; x++) {
	    		for (int y=0; y<height; y++) {
	    			int pix = source.getPixel(x,y);
	    			float[] hsb = new float[] {Image.giveandget(Image.pixelRed(pix)),
								Image.giveandget(Image.pixelGreen(pix)),
								Image.giveandget(Image.pixelBlue(pix))};
	    			for (int j=0; j<3; j++) {
						histogram[x][j]+=hsb[j];
    	...
    	// for debugging:
		// System.out.println("vs histogram: " + " H: " + histogram[width/2][0]
		// 	+ " S: " +  histogram[width/2][1]
		// 	+ " B: " + histogram[width/2][2]);
